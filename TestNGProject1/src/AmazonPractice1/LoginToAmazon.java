...
public class LoginToAmazon extends BaseClass {
	...
	public void login_ToAmazon() throws IOException, InterruptedException {
		...
		WebElement passwordPage = driver.findElement(By.xpath("//label[@for='ap_password']"));
		WebElement signinBtn = driver.findElement(By.id("signInSubmit"));
		try {
			if (passwordPage.isDisplayed()==true) {
			...
			System.out.println("Password page not found: " + e.getMessage());
		}

		WebElement otpPage = driver.findElement(By.xpath("//label[text()='Enter OTP']"));
		try {
			if (otpPage.isDisplayed()==true) {
				Thread.sleep(20000);
				signinBtn.click();
			}
		} catch (NoSuchElementException e) {
			System.out.println("OTP page not found: " + e.getMessage());
		}
